    ...
    fun chainCashDoubleSpendFailsWith() {
        ledger {
            ...
            transaction {
                ...
                output(Cash.PROGRAM_ID, "MEGA_CORP cash".output<Cash.State>().copy(owner = AnonymousParty(ALICE_PUBKEY)))
                command(MEGA_CORP_PUBKEY) { Cash.Commands.Move() }
                this.verifies()
            }
            ...
            tweak {
                transaction {
                    ...
                    output(Cash.PROGRAM_ID, "MEGA_CORP cash".output<Cash.State>().copy(owner = ALICE))
                    command(MEGA_CORP_PUBKEY) { Cash.Commands.Move() }
                    this.verifies()
                }
                this.fails()
